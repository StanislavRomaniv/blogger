...
const LikeComponent: FC<{ usersLikes: { email: string }[]; totalLikes: number; onChangeLike: Function; disabled: boolean }> = ({ usersLikes, totalLikes, onChangeLike, disabled }) => {
    const router = useRouter();
    const { data: session } = useSession();
    const [isSending, setIsSending] = useState(false);
    const [likeData, setLikeData] = useState({
        isLiked: usersLikes.map((item) => item.email === session?.user.email).length > 0 ? true : false,
    ...
    return (
        <button disabled={disabled} onClick={sendLikeHandler} className={styles.btn}>
            <svg className={likeData.isLiked ? styles.active : ''} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    ...
                    stroke={likeData.isLiked ? 'ff0000' : '#323232'}
                    strokeWidth="2"
                    strokeLinejoin="round"></path>
            </svg>
            <>{likeData.totalLikes > 0 ? likeData.totalLikes : ''}</>
            <span>Like</span>
        </button>
    );
};
